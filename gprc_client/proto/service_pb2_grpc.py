# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""Client and server classes corresponding to protobuf-defined services."""
import grpc

from proto import service_pb2 as proto_dot_service__pb2


class CountryServiceStub(object):
    """Missing associated documentation comment in .proto file."""

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.GetList = channel.unary_unary(
                '/CountryService/GetList',
                request_serializer=proto_dot_service__pb2.Empty.SerializeToString,
                response_deserializer=proto_dot_service__pb2.CountryListResponse.FromString,
                )
        self.UpdateCountry = channel.unary_unary(
                '/CountryService/UpdateCountry',
                request_serializer=proto_dot_service__pb2.CountryDTO.SerializeToString,
                response_deserializer=proto_dot_service__pb2.Empty.FromString,
                )


class CountryServiceServicer(object):
    """Missing associated documentation comment in .proto file."""

    def GetList(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def UpdateCountry(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_CountryServiceServicer_to_server(servicer, server):
    rpc_method_handlers = {
            'GetList': grpc.unary_unary_rpc_method_handler(
                    servicer.GetList,
                    request_deserializer=proto_dot_service__pb2.Empty.FromString,
                    response_serializer=proto_dot_service__pb2.CountryListResponse.SerializeToString,
            ),
            'UpdateCountry': grpc.unary_unary_rpc_method_handler(
                    servicer.UpdateCountry,
                    request_deserializer=proto_dot_service__pb2.CountryDTO.FromString,
                    response_serializer=proto_dot_service__pb2.Empty.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'CountryService', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))


 # This class is part of an EXPERIMENTAL API.
class CountryService(object):
    """Missing associated documentation comment in .proto file."""

    @staticmethod
    def GetList(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/CountryService/GetList',
            proto_dot_service__pb2.Empty.SerializeToString,
            proto_dot_service__pb2.CountryListResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def UpdateCountry(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/CountryService/UpdateCountry',
            proto_dot_service__pb2.CountryDTO.SerializeToString,
            proto_dot_service__pb2.Empty.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)
